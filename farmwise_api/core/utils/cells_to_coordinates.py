...
def s2cells_to_coordinates(pivoted_table):
    ...
    def normalize_cell(cell):
        if isinstance(cell, s2sphere.CellId) or hasattr(cell, "to_lat_lng"):
            return cell
        text = re.sub(r"\s*\([^)]*\)$", "", str(cell))
        match = re.search(r"CellId:\s*([0-9a-fA-F]+)", text)
        token = match.group(1) if match else text
        return s2sphere.CellId.from_token(token)

    s2_coordinates = [
        _s2cell_id_to_coordinate(normalize_cell(cell))
        for cell in s2cells
    ]
