...
so both entry points behave identically:
    ...
    * server mode  -> ``main.py`` / uvicorn, any CWD
    * library mode -> ``import farmwise``, any CWD
...
from core.utils.data_manifest import REMOTE_DATA
...
def _resolve_package_root() -> Path:
    """
    Locate the repository/package root that contains ``adapters/API_readers``.

    Walking up from this file rather than hard-coding ``parent.parent`` keeps
    this working if the package is later restructured (task 1.1) without
    needing a second edit here.
    """
    here = Path(__file__).resolve()
    for candidate in (here.parent, *here.parents):
        if (candidate / "adapters" / "API_readers").is_dir():
            return candidate
    # Fallback: repo-root layout (core/utils/paths.py -> repo root).
    return here.parents[2]


PROJECT_ROOT: Path = _resolve_package_root()
PACKAGE_ROOT: Path = PROJECT_ROOT

DATA_ROOT: Path = Path(
    os.environ.get(
        "FARMWISE_DATA_DIR",
        PROJECT_ROOT / "adapters" / "API_readers",
    )
).resolve()
